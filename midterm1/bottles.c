...
bottles_opts bottles_opts_new() {
    return (bottles_opts){.start = 99, .stop = 0};
}
...
char* pluralize(int64_t n) {
    if (n == 1) {
        return gettext("bottle");
    }
    return gettext("bottles");
}
...
void bottles(bottles_opts opts, FILE* out) {
    ...
    while (bottle != 0) {
        ...
        if (is_prime(bottle)) {
            ...
            // mathematician.
            fprintf(out, gettext("Take one down, pass it to the "
                                 "mathematician.\n"));
        } else {
    ...
    if (bottle == 0) {
        fprintf(out, gettext("No more bottles of beer on the wall, no more "
                             "bottles of beer.\n"));
        fprintf(out, gettext("Go to the store and buy some more, ninety-nine "
